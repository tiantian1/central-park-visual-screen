<!--  -->
<template>
  <div class="dragchartcontent"
       ref="dragcontnet">
    <vue-draggable-resizable :w="dragchartlist.widthlength"
      ...
      <ve-line :data="dragchartlist.chartData"
               width="100%"
               height="100%"
               :judge-width="true"
               ref="mychart"
               :extend="dragchartlist.extend"
               v-if="dragchartlist.type==1"></ve-line>
      <ve-ring :data="dragchartlist.chartData"
               width="100%"
               height="100%"
               :judge-width="true"
               ref="mychart"
               :settings="ringchartSettings"
               v-if="dragchartlist.type==2"></ve-ring>
      <ve-pie :data="dragchartlist.chartData"
              width="100%"
              height="100%"
              :judge-width="true"
              ref="mychart"
              :settings="piechartSettings"
              v-if="dragchartlist.type==3"></ve-pie>
      <ve-histogram :data="dragchartlist.chartData"
                    width="100%"
                    height="100%"
                    :judge-width="true"
                    ref="mychart"
                    v-if="dragchartlist.type==4"></ve-histogram>
      <ve-bar :data="dragchartlist.chartData"
              width="100%"
              height="100%"
              :judge-width="true"
              ref="mychart"
              v-if="dragchartlist.type==5"></ve-bar>
...
export default {
  ...
  watch: {
    ...
    dragchartlist: {
      ...
      handler (val, oldVal) {
        ...
        const length = this.dragchartlist.heightlength
  ...
  mounted () {
    const height = window.getComputedStyle(this.$refs.dragcontnet).height
    this.domheight = height.slice(0, this.domwidth.indexOf('p') - 1)
    const width = window.getComputedStyle(this.$refs.dragcontnet).width
